	...
	<div class="widget blue" id='goparameter'>
		...
		<div class="widget-body" id="parameter">
			<script language="javascript">
				loadform('includes/pnt/pnt_manager_form.php?lang=<?php echo $lang ?>','#parameter','#loadtext1');
			</script>
		</div>
	</div>
...
<script language="javascript">
	...
	$('#delbutton').live('click',function(){
		if(confirm('Are you sure?')) loadform('includes/pnt/pnt_manager_del.php?lang=<?php echo $lang ?>&id='+$(this).attr('qid'),$(this).parent(),$(this).parent());
	});
	
	$('#editbutton').live('click',function(){
		loadform('includes/pnt/pnt_manager_form.php?lang=<?php echo $lang ?>&lang=<?php echo $lang ?>&type=edit&id='+$(this).attr('qid'),'#parameter','#loadform');
		$(document).scrollTo('#goparameter',1000, {offset:-61})
	});
	
	$('#listenbutton').live('click',function(){
		$('#listenmusic').html('');
		loadform('includes/pnt/pnt_manager_popmusic.php?lang=<?php echo $lang ?>&id='+$(this).attr('qid'),'#listenmusic','#listenload');
	...
	$('.exam_r').live('change',function(){
		...
		//alert(djson);
		loadform("includes/pnt/pnt_manager_send.php?lang=<?php echo $lang ?>&djson="+djson,$(this));
	});
