	...
	echo "<tbody>";
		foreach($msgdata->list as $lists){
			...
			<tr id='row$row[id]'>
				...
				<td>$lists->sent_dt</td>
				<td>$lists->title</td>
				<td><a href="javascript:;" class='tooltips fulltext' data-original-title="$lists->content">[$lang_read]</a></td>
		</tr>
eot;
		}
	echo "</tbody>";
	...
	<div class="row-fluid">
		<div class="span6">
			<div class="dataTables_info">
				$lang_total <b>$msgdata->total_count</b> $lang_recordin <b>$ceil</b> $lang_pages, $lang_youarein <b>$page</b>.
			</div>
		</div>
		<div class="span6">
...
<script language='javascript'>
	$('.changepage').click(function(){
		loadform('includes/msg/msg_manager_db.php?lang=$lang&$requestparam&page='+$(this).attr('page')+'&limit=$limit','#entry','#pageloadtext');
	});
	
	$("select[name=limit]").change(function() {
		loadform('includes/msg/msg_manager_db.php?lang=$lang&$requestparam&page=1&limit='+$(this).val(),'#entry','#limitloadtext')
	});
