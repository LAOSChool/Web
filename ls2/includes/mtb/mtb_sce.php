	...
	<div class="widget blue" id='goparameter'>
		...
		<div class="widget-body" id="parameter">
			<script language="javascript">
				loadform('includes/mtb/mtb_sce_form.php?lang=<?php echo $lang ?>','#parameter','#loadtext1');
			</script>
		</div>
	</div>
</div>
...
<div class="span6">
	<div class="widget green">
		...
		<div class="widget-body" id="entry">
			<script language="javascript">
				loadform('includes/mtb/mtb_sce_db.php?lang=<?php echo $lang ?>','#entry','#loadtext');
			</script>
		</div>
	</div>
</div>
...
<script language="javascript">
	...
	$('#delbutton').live('click',function(){
		if(confirm('Are you sure?')) loadform('includes/mtb/mtb_sce_del.php?lang=<?php echo $lang ?>&id='+$(this).attr('qid'),$(this).parent(),$(this).parent());
	});
	
	$('#editbutton').live('click',function(){
		loadform('includes/mtb/mtb_sce_form.php?lang=<?php echo $lang ?>&type=edit&id='+$(this).attr('qid')+'&table='+$(this).attr('qtable'),'#parameter','#loadform');
		$(document).scrollTo('#goparameter',1000, {offset:-61})
	});
	
	$('#listenbutton').live('click',function(){
		$('#listenmusic').html('');
		loadform('includes/mtb/mtb_sce_popmusic.php?lang=<?php echo $lang ?>&id='+$(this).attr('qid'),'#listenmusic','#listenload');
	...
	$('.exam_r').live('change',function(){
		...
		//alert(djson);
		loadform("includes/mtb/mtb_sce_send.php?lang=<?php echo $lang ?>&djson="+djson,$(this));
	});
