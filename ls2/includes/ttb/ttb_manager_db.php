...
if($filter_class_id=='') die($lang_plsselectclass);
...
$sesdatas = explode("\n",$sesaip['output']);
$sesdata = json_decode(end($sesdatas));

//print_r($sesdata->messageObject->list);

$wkdaip = callapi($headers,'',$gets,'api/sys/sys_weekday');
$wkddatas = explode("\n",$wkdaip['output']);
$wkddata = json_decode(end($wkddatas));
...
foreach($trmdata->messageObject as $term_vals){
	...
	echo "<h3>$lang_term $term_vals->term_val</h3>";
		...
		for($j=1;$j<=7;$j++){
			...
			foreach($ttbdata->list as $lists){
				if($term_vals->term_val==$lists->term_val){
					if($lists->session_id==$sid && $lists->weekday_id==$j){
			...
			if($z==0) echo "<td id='addbutton' class='addbutton' qtable='$filter_class_id|$j|$sid|$term_vals->term_val'></td>";
		}
		echo "</tr>";
	}
	echo "</thead>";
...
<script language='javascript'>
	$('.changepage').click(function(){
		loadform('includes/ttb/ttb_manager_db.php?lang=$lang&$requestparam&page='+$(this).attr('page')+'&limit=$limit','#entry','#pageloadtext');
	});
	
	$("select[name=limit]").change(function() {
		loadform('includes/ttb/ttb_manager_db.php?lang=$lang&$requestparam&page=1&limit='+$(this).val(),'#entry','#limitloadtext')
	});
