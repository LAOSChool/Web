...
//print_r($ntfapi);
$ntfdatas = explode("\n",$ntfapi['output']);
$ntfdata = json_decode($ntfdatas[14]);
	...
	echo "<tbody>";
		foreach($ntfdata->list as $lists){
			...
			<tr id='row$row[id]'>
				...
				<td>$lists->sent_dt</td>
				<td>$lists->title</td>
				<td><a href="javascript:;" class='tooltips fulltext' data-original-title="$lists->content <hr> $imgcontent">[Read]</a></td>
		</tr>
eot;
		}
	echo "</tbody>";
...
<script language='javascript'>
	$('.changepage').click(function(){
		loadform('includes/ntf/ntf_manager_db.php?$requestparam&page='+$(this).attr('page')+'&limit=$limit','#entry','#pageloadtext');
	});
	
	$("select[name=limit]").change(function() {
		loadform('includes/ntf/ntf_manager_db.php?$requestparam&page=1&limit='+$(this).val(),'#entry','#limitloadtext')
	});
