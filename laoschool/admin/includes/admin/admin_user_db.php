...
eot;
	echo "<tbody>";
	$res = $db->query("select * from users where ($namequery) order by id desc limit $pages,$limit");
	...
	$rrr = $db->getone("select count(*) from users where ($namequery) and ($cpidquery)");
