	...
	echo "<tbody>";
		foreach($usrdata->list as $lists){
			...
			echo "<tr>
				...
				<td>$lists->roles</td>
				<td>$class</td>
				<td align=center width=20><a title='$lang_rspass' class='tooltips btn btn-mini btn-info' id='passbutton' qtable='$filter_user_role' qid='$lists->id' href='javascript:void(0)'><i class='icon-key'></a></td>
				<td align=center width=20><a title='$lang_edit' class='tooltips btn btn-mini btn-warning' id='editbutton' qtable='$filter_user_role' qid='$lists->id' href='javascript:void(0)'><i class='icon-edit'></a></td>
			</tr>";
		}
	echo "</tbody>";
	echo "</table>";
...
<script language='javascript'>
	$('.changepage').click(function(){
		loadform('includes/usr/usr_manager_db.php?lang=$lang&$requestparam&page='+$(this).attr('page')+'&limit=$limit','#entry','#pageloadtext');
	});
	
	$("select[name=limit]").change(function() {
		loadform('includes/usr/usr_manager_db.php?lang=$lang&$requestparam&page=1&limit='+$(this).val(),'#entry','#limitloadtext')
	});
