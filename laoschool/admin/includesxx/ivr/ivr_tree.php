<div class="span6">
	<div class="widget green">
		<div class="widget-title">
			<h4><i class="<?php echo $param_icon; ?>"></i> <?php echo $currentsubtab ?></h4>
			<span class="tools">
		...
		<div class="widget-body" id="entry">
			<script language="javascript">
				loadform('includes/ivr/ivr_tree_db.php','#entry','#loadtext');
			</script>
		</div>
	</div>
</div>
...
<div class="span6">
	<div class="widget blue" id='goparameter'>
		...
		<div class="widget-body" id="parameter">
			<script language="javascript">
				loadform('includes/ivr/ivr_tree_form.php','#parameter','#loadtext1');
			</script>
		</div>
	</div>
</div>
...
<div id="modalform" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="modalform" aria-hidden="true">
	<div class="modal-header">
		...
		<h3 id="modalform">Listen music</h3>
	...
	<div class="modal-footer">
		<span id='listenload'>Loading...</span> 
		<button data-dismiss="modal" class="btn btn-danger" onclick="$('#html5player')[0].pause();"><?php lang('close') ?></button>
	</div>
</div>
...
<script language="javascript">
	...
	$('#delbutton').live('click',function(){
		if(confirm('Are you sure?')) loadform('includes/ivr/ivr_tree_del.php?id='+$(this).attr('qid'),$(this).parent(),$(this).parent());
	});
	
	$('#editbutton').live('click',function(){
		loadform('includes/ivr/ivr_tree_form.php?lang=<?php echo $lang ?>&type=edit&id='+$(this).attr('qid'),'#parameter','#loadform');
		$(document).scrollTo('#goparameter',1000, {offset:-61})
	});
	
	$('#listenbutton').live('click',function(){
		$('#listenmusic').html('');
		loadform('includes/ivr/ivr_tree_popmusic.php?id='+$(this).attr('qid'),'#listenmusic','#listenload');
