<div class="span12">
	<div class="widget blue" id='goparameter'>
		...
		<div class="widget-body" id="parameter">
			<script language="javascript">
				loadform('includes/pnt/pnt_manager_form.php','#parameter','#loadtext1');
			</script>
		</div>
	</div>
	<div class="widget green">
		...
		<div class="widget-body" id="entry">
			<script language="javascript">
				loadform('includes/pnt/pnt_manager_db.php','#entry','#loadtext');
			</script>
		</div>
	</div>
</div>
...
<script language="javascript">
	...
	$('#delbutton').live('click',function(){
		if(confirm('Are you sure?')) loadform('includes/pnt/pnt_manager_del.php?id='+$(this).attr('qid'),$(this).parent(),$(this).parent());
	});
	
	$('#editbutton').live('click',function(){
		loadform('includes/pnt/pnt_manager_form.php?lang=<?php echo $lang ?>&type=edit&id='+$(this).attr('qid'),'#parameter','#loadform');
		$(document).scrollTo('#goparameter',1000, {offset:-61})
	});
	
	$('#listenbutton').live('click',function(){
		$('#listenmusic').html('');
		loadform('includes/pnt/pnt_manager_popmusic.php?id='+$(this).attr('qid'),'#listenmusic','#listenload');
	...
	$('.exam_r').live('change',function(){
		...
		//alert(djson);
		loadform("includes/pnt/pnt_manager_send.php?djson="+djson,$(this));
	});
