...
<?php elseif($type=='add'): ?>
	<form method=POST id='managerform' class="form-horizontal" action='includes/sadmin/sadmin_user_add.php'>	
		...
		<div class="control-group com_id">
			<label class="control-label">Company</label>
			<div class="controls">
				<select name='com_id'>
					<?php
					$res = $db->query("select * from companies order by name asc");
					while($comrow = $res->fetchRow()){
						echo "<option value='$comrow[id]'>$comrow[name]</option>\n";
					};
					?>
				</select>
			</div>
		...
		<div class="control-group com_id">
			...
			<div class="controls">
				<select name='com_id'>
					...
					$res = $db->query("select * from companies order by name asc");
					while($comrow = $res->fetchRow()){
						$selected = ($comrow['id']==$row['com_id'])?"selected":"";
						echo "<option $selected value='$comrow[id]'>$comrow[name]</option>\n";
					};
					?>
				</select>
			</div>
...
<script language="javascript">
	...
	<?php if($row['role']=='sadmin'): ?>
		$('.com_id').hide();
	<?php elseif($row['role']=='admin'): ?>
		$('.com_id').show();
	<?php elseif($row['role']=='mod'): ?>
		$('.com_id').show();
	<?php endif; ?>
	
	$('input[name=role]').change(function(){
		if($(this).val()=='sadmin' || $(this).val()=='report'){
			$('.com_id').hide();
	...
	$('.formchange').click(function(){
		loadform('includes/sadmin/sadmin_user_form.php?type='+$(this).attr('form'),'#parameter','#loadform');
	})
