...
<head>
	...
	<link rel="stylesheet" type="text/css" href="css/style-gray.css" id="style_color" />
	...
	<script type="text/javascript" src="assets/bootstrap-inputmask/jquery.inputmask.js"></script>
...
<?php
	if( $_SESSION[$config_session]['loggedin'] == 'true' ){
		...
		$usernamex = $_SESSION[$config_session]['username'];
		$permissionx = $_SESSION[$config_session]['permission'] ;
		$rolex = $_SESSION[$config_session]['role'] ;
	}else $logged = '';
	...
	<div class="navbar-inner">
		<div class="container-fluid">
			...
			<a class="brand" href="index.php">
			   <img src="img/logo.png" alt="<?php echo $webtitle ?>" />
			</a>
			...
			<div id="top_menu" class="nav notify-row">
				<ul class="nav top-menu" >
					<li class="dropdown" id="header_inbox_bar">
                           <?php
								$mailcount = $db->getone("select count(*) from vhs_cobtact where isread=0");
						   ?>
						   <a href="#" class="dropdown-toggle" data-toggle="dropdown">
                               <i class="icon-envelope-alt"></i>
                               <span class="badge badge-important"><?php echo $mailcount; ?></span>
                           </a>
							<ul class="dropdown-menu extended inbox">
								<li>
								   <p>Bạn có <?php echo $mailcount; ?> liên hệ chưa đọc</p>
								</li>
								<?php
									$mres = $db->query("select *,date(datetime) as date from vhs_cobtact where isread=0 order by `datetime` desc");
									while($mres->fetchInto($mrow)){
										$message = truncate_str($mrow['message'],20);
										echo<<<eot
										<li>
											<a href="$dirfile/?act=contact#$mrow[id]">
											
												<span class="subject">
													<span class="from">$mrow[name]</span>
													<span class="time">$mrow[phone]</span>
												</span>
												<span class="message">
													$message
												</span>
...
eot;
								...
								<li>
								   <a href="<?php echo "$dirfile/?act=contact"?>">Xem toàn bộ liên hệ</a>
								</li>
							</ul>
						</li>
					</ul>
			...
			<div class="top-nav ">
				...
				<ul class="nav pull-right top-menu" >
					...
					<li class="dropdown" id="header_notification_bar">
						<a href="index.php?act=config" class="dropdown-toggle">
							<i class="icon-cogs"></i>
							<span ></span>
						</a>
					</li>
					<li class="dropdown">
						<a href="#" class="dropdown-toggle" data-toggle="dropdown">
							...
							<span class="username"><?php echo strtoupper($_SESSION[$config_session]['username']); ?></span>
						</a>
						<ul class="dropdown-menu extended logout">
							<?php if($_SESSION[$config_session]['username']=='admin'): ?>
								<li><a href="index.php?act=admin"><i class="icon-group"></i> Quản lý tài khoản</a></li>
							<?php endif; ?>
							<li><a href="index.php?act=acc"><i class="icon-user"></i> Thông tin cá nhân</a></li>
							<li><a href="index.php?act=login&logout=1"><i class="icon-signout"></i> Đăng xuất</a></li>
						</ul>
					</li>
				</ul>
			</div>
							...
							if($permissionx=='*' && $tab[2]==='0'){
								...
								if(is_array($$subtab) && count($$subtab)>1){
									foreach($$subtab as $sub){
										...
										$subcontent.="<li class='$current_sub_page_item'><a class='' href='?act=$tab[0]&go=$sub[0]'>$sub[1]</a></li>\n";
									};
